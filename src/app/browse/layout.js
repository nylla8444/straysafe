...
export default function BrowseLayout({ children }) {
    ...
    return (
        <div className="flex min-h-screen">
            {/* Sidebar */}
            <div className={`bg-white shadow-md transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-16'}`}>
                ...
                <nav className="mt-5">
                    <ul>
                        <li>
                            <Link
                                href="/browse/pets"
                                className={`flex items-center p-4 ${pathname.includes('/browse/pets') ? 'bg-blue-100 text-blue-600' : 'text-gray-700 hover:bg-gray-100'}`}
                            >
